...
extension TerminalView {
    ...
    func updateCursorPosition()
    {
        ...
        if vy >= buffer.yDisp + buffer.rows {
            ...
            // GALACTIC: Same detach-cancels-the-animation problem as
            // `showCursor` — this is the other path that puts the caret back,
            // so it carries the same restore. Reached when the cursor comes
            // back on screen rather than when the program re-shows it.
            caretView.updateCursorStyle()
    ...
    /// Recompute the caret's frame from the current cursor cell and viewport.
    /// Public so a Galactic subclass can force a reposition after it mutates
    /// the viewport (e.g. snapping to the live bottom on focus), where the
    /// internal display cycle would otherwise leave the caret frame stale
    /// until the next output-driven updateDisplay.
    public func repositionCaret() {
        updateCursorPosition()
    }
    ...
    public func scrollTo (row: Int, notifyAccessibility: Bool = true)
    {
        ...
        if row != displayBuffer.yDisp {
            ...
            // GALACTIC: reconcile gesture-sourced follow intent on EVERY scroll
            // movement, UNGATED by selection — so scrolling back to the bottom
            // during an active selection still clears it. (The userScrolling
            // line below is skipped during selection, which is exactly what
            // stranded the follow gate.) Only real scroll gestures reach
            // scrollTo, so output drift never sets this.
            terminal.scrolledUpByUser = displayBuffer.yDisp < displayBuffer.yBase
    ...
    {
        search.invalidate()
        // GALACTIC: Recover a userScrolling flag stranded true by a
        // transient selection. The authoritative signal is
        // scrolledUpByUser (set only by real scroll gestures): if the
        // user never gesture-scrolled up, a true userScrolling is stale
        // and is cleared here so Terminal.scroll() re-pins yDisp to
        // yBase during this feed. Keying off intent rather than position
        // heals the stranding even after drift has already opened up
        // (yDisp < yBase) — the case the old `yDisp >= yBase` guard could
        // never recover from. Still gated on !selection.active, so an
        // active selection keeps the viewport frozen (invariant 2).
        if !selection.active && terminal.userScrolling && !terminal.scrolledUpByUser {
            terminal.userScrolling = false
        }
